...
function wpcrown_comment($comment, $args, $depth) 
{
	...
	<div class="comment_block" id="comment-<?php comment_ID() ?>">
        ...
        <div class="comment-header">
			...
			<div class="three-fourth" style="margin-bottom: 10px;">
				<a href="<?php echo $comment->comment_author_url; ?>"><h4><?php echo $comment->comment_author; ?></h4></a>
			</div>	
			<p>Posted on <?php echo mysql2date('l jS F, Y, g:ia', $comment->comment_date); ?></p>
		</div>	
	...
	if(!empty($posts))
	{
		...
		$return_html.= '<ul>';
			...
			foreach($posts as $post)
			{
				...
				$return_html.= '<li>';
				$return_html.= '<div class="header"><a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a></div><p>' .get_the_time('F jS, Y'). '</p></li>';
