...
Description: A plugin to generate shortcodes in the WordPress visual editor. <strong>Use specific in Circle Free theme</strong>.
Version: 1.0
Author: Kopatheme
...
function kopa_shortcode_contact_form($atts, $content = null) {
    ...
    $out .= '<label class="required" for="contact_name">' . __('Name (required):', 'kopa-shortcodes') . '</label>';
    $out .= '<input class="valid" type="text" name="name" id="contact_name" value="">';
    $out .= '</p>';

    $out .= '<p class="input-block clearfix">';
    $out .= '<label class="required" for="contact_email">' . __('Email (required):', 'kopa-shortcodes') . '</label>';
    ...
    $out .= '<label class="required" for="contact_message">' . __('Message (required):', 'kopa-shortcodes') . '</label>';
    $out .= '<textarea id="contact_message" name="message"></textarea>';
    $out .= '</p>';

    $out .= '<p class="comment-button clearfix">';
    $out .= '<input type="submit" id="submit-contact" value="' . __('Send', 'kopa-shortcodes') . '">';
    $out .= '</p>';
    ...
    if ($atts['address'] || $atts['phone'] || $atts['email']) {
        ...
        $out.= '<h2 class="contact-title">' . __('Contact Information', 'kopa-shortcodes') . '</h2>';
        $out.= '<address>';
        if ($atts['address'])
