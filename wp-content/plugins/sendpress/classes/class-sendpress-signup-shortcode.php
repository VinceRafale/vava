...
class SendPress_Signup_Shortcode {
    ...
    static function load_form($attr, $content = null) {
        ...
        if (count($list_ids) > 1) {
            ?>
                        <p>
                            ...
                            <?php foreach ($list_ids as $id) { ?>
                                <input type="checkbox" name="sp_list[]" class="sp_list" id="list<?php echo $id; ?>" value="<?php echo $id; ?>" <?php if ($lists_checked) {
                    echo 'checked';
                    ...
                    ?>
                        ...
                        <?php endif; ?>
                            <input type="text" class="sp_firstname" orig="<?php echo $firstname_label; ?>" value="<?php if ($label) {
            ...
            <?php if (!$label): ?>
                            ...
                            <?php endif; ?>
                            <input type="text" class="sp_lastname" orig="<?php echo $lastname_label; ?>" value="<?php if ($label) {
                    echo $lastname_label;
                } ?>" name="sp_lastname" />
                        </p>
                        <?php endif; ?>
        ...
        <?php endif; ?>
                        <input type="text" class="sp_email" orig="<?php echo $email_label; ?>" value="<?php if ($label) {
            echo $email_label;
        } ?>" name="sp_email" />
                    </p>
        ...
        <?php do_action('sendpress_signup_form_bottom'); ?>
					...
					<div id="error"><?php echo $sendpress_signup_error; ?></div>
									<div id="thanks" <?php if ($sendpress_show_thanks) {
								echo 'style="display:block;"';
							} else {
								echo 'style="display:none;"';
							} ?>><?php echo $thank_you; ?></div>
									<div id="form-wrap" <?php if ($sendpress_show_thanks) {
								echo 'style="display:none;"';
							} ?>> 
                </div>
            </form>
